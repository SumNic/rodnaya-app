...
export class AppMessagesController {
    constructor(@Inject(AUTH_SERVICE) private messagesClient: ClientProxy, private configService: ConfigService) {}
    ...
    @UseGuards(JwtAuthGuard)
    async getNextMessage(@Query() query: any) {
        return this.messagesClient.send('getNextMessages', query).pipe(
            catchError(async (error) => {
                return new RpcException(error);
            }),
        );
    ...
    @UseGuards(JwtAuthGuard)
    async getPreviousMessage(@Query() query: any) {
        return this.messagesClient.send('getPreviousMessages', query).pipe(
            catchError(async (error) => {
                return new RpcException(error);
            }),
        );

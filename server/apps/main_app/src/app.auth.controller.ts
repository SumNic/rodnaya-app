...
export class AppAuthController {
    ...
    /**
     * Конечная точка для выхода из системы пользователя.
     * Удаляет refreshToken пользователя из базы данных.
     *
     * @param dto - Объект, содержащий идентификатор пользователя.
     * @returns - HTTP код состояния 200, если пользователь успешно вышел из системы.
     * @throws - HTTP код состояния 404, если пользователь не найден.
     * @throws - HTTP код состояния 401, если JWT токен не указан или недействителен.
     */
    @ApiTags('Авторизация')
    @ApiOperation({
    ...
    async logout(@Body() dto: LogoutUserDto) {
        return this.authClient.send('logout', dto).pipe(
            catchError(async (error) => {
                return new RpcException(error);
            }),
        );
    ...
    // })
    async getAllUsers() {
        return this.authClient.send('getAllUsers', []).pipe(
            catchError(async (error) => {
                return new RpcException(error);
            }),
        );
    ...
    async registrationVk(@Body() dto: VkLoginSdkDto) {
        return this.authClient.send('loginByVk', dto).pipe(
            catchError(async (error) => {
                return new RpcException(error);
            }),
        );
    ...
    async userAddRole(@Body() dto: AddRoleDto) {
        return this.authClient.send('userAddRole', dto).pipe(
            catchError(async (error) => {
                return new RpcException(error);
            }),
        );
    ...
    async userRemoveRole(@Body() dto: AddRoleDto) {
        return this.authClient.send('userRemoveRole', dto).pipe(
            catchError(async (error) => {
                return new RpcException(error);
            }),
        );
    ...
    async getAllRoles() {
        return this.authClient.send('getAllRoles', {}).pipe(
            catchError(async (error) => {
                return new RpcException(error);
            }),
        );
    ...
    async createResidency(@Body() dto: CreateResidencyDto) {
        return this.authClient.send('createResidency', dto).pipe(
            catchError(async (error) => {
                return new RpcException(error);
            }),
        );
    ...
    async getAllResydencys() {
        return this.authClient.send('getAllResydencys', []).pipe(
            catchError(async (error) => {
                return new RpcException(error);
            }),
        );
    ...
    async addDeclaration(@Body() form: any) {
        return this.authClient.send('addDeclaration', form).pipe(
            catchError(async (error) => {
                return new RpcException(error);
            }),
        );
    ...
    async getDeclaration(@Param('id') id: number) {
        return this.authClient.send('getDeclaration', id).pipe(
            catchError(async (error) => {
                return new RpcException(error);
            }),
        );

...
export class AppFilesController {
    constructor(@Inject(AUTH_SERVICE) private filesClient: ClientProxy) {}
    ...
    async uploadFile(@UploadedFile() file: Express.Multer.File) {
        ...
        if (!isType)
            return new RpcException(
                new BadRequestException(
                    'Допустимы расширения: jpg, png, jpeg, pdf, doc, rtf, odt',
                ),
            );
        return this.filesClient.send('saveFile', file).pipe(

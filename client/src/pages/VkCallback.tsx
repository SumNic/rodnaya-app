...
import { BLOCKED_ROUTE, PERSONALE_ROUTE, REGISTRATION_ROUTE, RESTORE_PROFILE_ROUTE } from '../utils/consts';
...
const VkCallback: React.FC = () => {
	...
	async function registration(payload: {}) {
		...
		if (registrVk.data.isDelProfile) {
			...
			await store.authStore.loginVk(registrVk.data.id, registrVk.data.secret);
